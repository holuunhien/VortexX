    ...
    function hideButtons() {
        ...
        labels.forEach(label => {
            ...
            if (
                ...
                text.includes('theme') ||
                text.includes('score') ||
                text.includes('fix lag') ||
                text.includes('no name')
          ) {
...
(function () {
    ...
    function removeSettingsPanel() {
        ...
        if (panel) {
            ...
            panel.remove(); // xoá luôn khỏi DOM
        }
    }
    ...
    // game reload UI liên tục nên phải canh
    const observer = new MutationObserver(removeSettingsPanel);
...
function createMiniMapText() {

    if (document.getElementById("luunhien-text")) return;

    const text = document.createElement("div");
    text.id = "luunhien-text";
    text.innerText = "Luu Nhien";

    // STYLE chữ nhạt
    text.style.position = "fixed";
    text.style.right = "90px";
    text.style.bottom = "90px";
    text.style.transform = "translate(50%,50%)";
    text.style.pointerEvents = "none";

    text.style.color = "rgba(255, 80, 80, 0.45)"; // ⭐ màu đỏ
    text.style.fontSize = "16px";
    text.style.fontWeight = "600";
    text.style.textShadow = "0 0 4px rgba(0,0,0,0.4)";
    text.style.zIndex = "9999";

    document.body.appendChild(text);
}

setTimeout(createMiniMapText, 3000);

})();
(function () {
    'use strict';

    function removeSpamUI() {

        // icon email
        const email = document.querySelector('img[src*="mail"]');
        if (email) email.parentElement.remove();

        // nút nhận ngay (bao lì xì)
        const giftBtn = [...document.querySelectorAll("img")]
            .find(el => el.src.includes("lixi") || el.src.includes("gift"));

        if (giftBtn) giftBtn.parentElement.remove();

        // text "nhớ mở bao lì xì"
        const tip = [...document.querySelectorAll("div,span")]
            .find(el => el.innerText && el.innerText.includes("bao lì xì"));
    ...
    const observer = new MutationObserver(removeSpamUI);
    observer.observe(document.body, {
        childList: true,
        subtree: true
    });
...
(function(){
    ...
    function newIns(){
        ...
        if (skinInput) {
            skinInput.value = ""; // thay id skin o day !
        }
